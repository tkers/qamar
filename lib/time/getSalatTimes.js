"use strict";

// dependencies
const getJulianDate = require("./getJulianDate");
const TimeUtils = require("./TimeUtils");
const normalizeHours = TimeUtils.normalizeHours;
const SunUtils = require("./SunUtils");

const calculateTimes = opts => {

    // get options
    const now = opts.date || new Date();
    const lngOffset = opts.longitude / 15.0;
    const lat = opts.latitude;
    const timezone = opts.timezone || -now.getTimezoneOffset() / 60.0;
    let precision = opts.precision || 3;

    const angles = opts.angles;
    const asrShadowLength = opts.asrShadowLength;
    const getNightPortion = opts.highLatitudeCorrection;
    const imsaakAsOffset = typeof angles.imsaakOffset !== "undefined";
    const dhuhrAsOffset = typeof angles.dhuhrOffset !== "undefined";
    const maghribAsOffset = typeof angles.maghribOffset !== "undefined";
    const ishaAsOffset = typeof angles.ishaOffset !== "undefined";

    // init
    const julianDate = getJulianDate(now) - lngOffset / 24.0;
    const sunCalc = SunUtils.init(julianDate, lat);

    let times = Array(8).fill(0);

    // Calculate times
    while (precision --> 0) {

        if (!imsaakAsOffset)
            times[0] = sunCalc.computeTime(180 - angles.imsaakAngle, times[0]);
        times[1] = sunCalc.computeTime(180 - angles.fajrAngle, times[1]);
        times[2] = sunCalc.computeTime(SunUtils.SUNRISE_ANGLE, times[2]);
        times[3] = sunCalc.computeMidday(times[3]);
        times[4] = sunCalc.computeAsr(asrShadowLength, times[4]);
        times[5] = sunCalc.computeTime(SunUtils.SUNSET_ANGLE, times[5]);
        if (!maghribAsOffset)
            times[6] = sunCalc.computeTime(angles.maghribAngle, times[6]);
        if (!ishaAsOffset)
            times[7] = sunCalc.computeTime(angles.ishaAngle, times[7]);
    }

    // Add offsets
    if (imsaakAsOffset)  times[0] = times[1] - angles.imsaakOffset / 60.0;
    if (dhuhrAsOffset)   times[3] += angles.dhuhrOffset / 60.0;
    if (maghribAsOffset) times[6] = times[3] + angles.maghribOffset / 60.0;
    if (ishaAsOffset)    times[7] = times[4] + angles.ishaOffset    / 60.0;

    // Adjust for timezone
    times = times.map(t => t + timezone - lngOffset);

    // Adjust for high latitudes
    if (typeof getNightPortion !== "undefined") {
        const nightTime = normalizeHours(times[2] - times[5]);

        const fajrDiff = nightTime * getNightPortion(angles.fajrAngle);
        const ishaDiff = nightTime * getNightPortion(ishaAsOffset ? 18.0 : angles.ishaAngle);
        const maghribDiff = nightTime * getNightPortion(maghribAsOffset ? 4.0 : angles.maghribAngle);

        if (isNaN(times[1]) || normalizeHours(times[2] - times[1]) > fajrDiff)
            times[1] = times[2] - fajrDiff;
        if (isNaN(times[6]) || normalizeHours(times[6] - times[5]) > maghribDiff)
            times[6] = times[5] + maghribDiff;
        if (isNaN(times[7]) || normalizeHours(times[7] - times[5]) > ishaDiff)
            times[7] = times[5] + ishaDiff;
    }

    // normalize to 24h
    return times.map(normalizeHours);
};

module.exports = calculateTimes;
